...
public class LockerService {
    ...
    public ResultStr rentLocker() {
        ResultInt rentableLocker = checkLockerAvailability(); // Returns a ResultInt that store the index of a locker and if one is available.
        if (rentableLocker.getStatus()) { // If there is at least one rentable locker
            int freeLockerIndex = rentableLocker.getNumber(); // Index of earliest locker that can rented.
            String lockerPIN = PinUtils.generatePIN(); // Generate a random 4-digit PIN String.
    ...
    public ResultStr isValidLockerID(int lockerID) {
        if ((lockerID >= 1 && lockerID <= lockers.length)) {
            if (lockers[lockerID - 1] == null) {
                // Checks if the locker is currently not being rented. (Goes from 1-based index to 0)
